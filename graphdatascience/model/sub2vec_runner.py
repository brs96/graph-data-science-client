...
class Sub2VecRunner(UncallableNamespace, IllegalAttrChecker):
    def __init__(
        self,
    ...
    @compatible_with("train_and_stream", min_inclusive=ServerVersion(2, 5, 0))
    @client_only_endpoint("gds.sub2vec")
    ...
    @compatible_with("predict_and_stream", min_inclusive=ServerVersion(2, 5, 0))
    @client_only_endpoint("gds.sub2vec")
    def predict_and_stream(
        self,
        G: Graph,
        model_name: str,
        mlflow_experiment_name: Optional[str] = None,
    ...
    def _wait_for_job(self, job_id: str) -> None:
        while True:
            ...
            if res_json["job_status"] == "exited":
                logging.info("Sub2Vec job completed!")
                return
            elif res_json["job_status"] == "failed":
                error = f"Sub2Vec job failed with errors:{os.linesep}{os.linesep.join(res_json['errors'])}"
    ...
    def _stream_results(self, job_id: str) -> DataFrame:
        ...
        upload_descriptor = pa.flight.FlightDescriptor.for_path(f"{job_id}.graphs")
